...
function BusinessWebsitesPage() {
  ...
  return <PageShell>
    ...
    <EvidencePlate id="owned-demo-mosaic" />
...
function OngoingSeoPage() {
  ...
  return <PageShell>
    <ServiceHero route={route} eyebrow="ONGOING ORGANIC SEO & LOCAL GROWTH · FROM $450/MONTH" title="Ongoing organic SEO—with implementation, not just reports." copy="Each cycle, Boho agrees on one priority and does the technical, content, internal-linking, local-visibility, analytics, or customer-path work needed to move it forward." price="From $450/month" receipt="One named priority · Implementation · Completed-work record" />
    <section className="reset-section" id="local-seo"><div className="reset-shell reset-service-detail__split"><article><p className="reset-eyebrow">The fit</p><h2>Who it is for</h2><p>Businesses with a useful reason to keep improving how customers discover, understand, and act on the website.</p><p>This is implementation work supported by Boho&apos;s in-house analytics software—not a monthly PDF that substitutes activity for progress.</p></article><article id="customer-paths-and-conversion-clarity"><p className="reset-eyebrow">The recurring scope</p><h2>What is included</h2><ul>{seoIncluded.map((item) => <li key={item}>{item}</li>)}</ul></article></div></section>
    <EvidencePlate id="gsc-opportunity" />
    ...
    <AnalyticsPlatformSpotlight context="seo" />
...
function CustomSystemsPage() {
  ...
  return <PageShell>
    ...
    <EvidencePlate id="custom-systems-workbench" />
